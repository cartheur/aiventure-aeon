...
{
    /// <summary>
    /// The topicstar element tells the  interpreter that it should substitute the contents of 
    ...
    /// </summary>
    public class TopicStar : CoreTagHandler
    {
        /// <summary>
        ...
        public TopicStar(Aeon thisAeon,
                        User thisUser,
                        SubQuery query,
                        Request thisRequest,
                        Result thisResult,
        ...
        protected override string ProcessChange()
        {
            if (TemplateNode.Name.ToLower() == "topicstar")
            {
                if (TemplateNode.Attributes != null && TemplateNode.Attributes.Count == 0)
                {
                    ...
                    ThisAeon.WriteToLog("An out-of-bounds index to topicstar was encountered when processing the input: " + ThisRequest.RawInput);
                }
                else if (TemplateNode.Attributes != null && TemplateNode.Attributes.Count == 1)
                {
                    if (TemplateNode.Attributes[0].Name.ToLower() == "index")
                    {
                        if (TemplateNode.Attributes[0].Value.Length > 0)
                        {
                            ...
                            {
                                ThisAeon.WriteToLog("A thatstar tag with a baldy formed index (" + TemplateNode.Attributes[0].Value + ") was encountered processing the input: " + ThisRequest.RawInput);
                            }
                        }
                    }
                }
